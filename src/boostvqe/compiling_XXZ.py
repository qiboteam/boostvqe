...
def nqubit_XXZ_decomposition(nqubits, t, delta=0.5, steps=1, order=1):
    ...
    if order == 1:
        if nqubits % 2 == 0:
            ...
            circuit = _add_gates(
                circuit, odd_numbers, even_numbers_end_0, t, delta, steps
            )
        elif nqubits % 2 == 1:
            # Handle odd number of qubits (since XXZ model has periodic boundary conditions)
            circuit = _add_gates(
                circuit, even_numbers[:-1], odd_numbers, t, delta, steps
            )
            circuit = _add_gates(
                circuit, odd_numbers, even_numbers[1:], t, delta, steps
            )
        ...
        if nqubits % 2 == 0:
            # Handle even number of qubits
            circuit = _add_gates(
                circuit, odd_numbers, even_numbers_end_0, t / 2, delta, steps
            )
            circuit = _add_gates(circuit, even_numbers, odd_numbers, t, delta, steps)
            circuit = _add_gates(
                circuit, odd_numbers, even_numbers_end_0, t / 2, delta, steps
            ...
            # Handle odd number of qubits (since XXZ model has periodic boundary conditions)
            circuit = _add_gates(
                circuit, odd_numbers, even_numbers[1:], t / 2, delta, steps
            )
...
def vw_xxz_compiling_phase(nqubits, steps, order=1):
    if order == 1:
        ...
        if nqubits % 2 == 0:
            ...
            return np.exp(
                1j * np.pi / 4 * nqubits * steps
                + 1j * np.pi / 4 * (nqubits + 1) * steps / 2
            )
