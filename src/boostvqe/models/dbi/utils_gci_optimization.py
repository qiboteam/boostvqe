...
def dephasing_approximation(h_matrix: np.array, order=1):
    ...
    order_1 = [
        diagonal.T @ diagonal_1_pauli_z(i, nqubits) / 2**nqubits
        for i in range(nqubits)
    ]
    order_2 = [
        diagonal.T @ diagonal_product_pauli_z([i, j], nqubits) / 2**nqubits
        for i, j in itertools.product(range(nqubits), range(nqubits))
    ]
...
def nn_dephasing_approximation(h_matrix: np.array, order=1):
    ...
    order_2 = [
        diagonal.T @ diagonal_product_pauli_z([i, i + 1], nqubits) / 2**nqubits
        for i in range(nqubits)
    ]
    return {1: order_1, 2: order_2}
...
def gradient_descent_circuits(
    ...
    for i in range(train_epochs):
        ...
        step_guess, loss, _ = dbi_object.choose_step(
            d=d, times=times_choose_step, verbose=False
        )
        print(loss)
...
def gradient_descent_circuits_lr(
    ...
    d_init = params_to_diagonal_operator(
        d_params_init, nqubits, parameterization=ParameterizationTypes.circuits
    )
    ...
    for i in range(train_epochs):
        ...
        for lr in np.linspace(1, 1e3, 9):
            d_params_test = [d_params_init[j] - grad[j] * lr for j in range(len(grad))]
            d = params_to_diagonal_operator(
                d_params_test, nqubits, parameterization=ParameterizationTypes.circuits
            )
...
def get_gd_evolution_oracle(n_local, params):
    if n_local == 1:
        ...
        raise_error(
            ValueError, "Model not supported, change `n_local` to values in [1, 2]"
        )
...
def evaluate_histogram_data(
    gci_object, input_field, nmb_shots=10, randomization_amp=0.2
):
    ...
    for j in range(nmb_shots):
        field = [
            a + b
            for a, b in zip(
                np.random.rand(len(input_field)) * randomization_amp, input_field
            )
        ]
        eo_d = MagneticFieldEvolutionOracle(field, name="D(linear)")
        step, loss, _ = gci_object.choose_step(
            d=eo_d, max_evals=34, step_min=0.0051, step_max=0.03
        )
        losses.append(loss)
        fields.append(field)
        steps.append(step)
